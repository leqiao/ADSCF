...
!
    ...
    monomer_typeB = 2
    block_lengthA = 0.5d0
    block_lengthB = 1.d0- block_lengthA
!    polymer_ensemble  = 1  ! canonical
    ...
    ! write(*,*)'./Data/energy_lx'//trim(arg_lx)//'_h'//trim(arg_lz)//'_f'//trim(arg_ff)//'.dat'
...
!--------DENSITY_REFERENCE
!!
!  if (i_sizex .eq. 3) then
!      density_ref = saddledensity
!  end if

! open(unit=3,file='initial_den.dat',status='old',action='read',iostat=io_error)
! do x=1,gridx
!    do y=1,gridy
!       do z=1,gridz
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
  ...
  do x = 1,gridx
     do y = 1,gridy
        do z = 1,gridz-1
        density(x,y,z,1) = block_lengthA*(1.0d0 + 0.5d0*cos(dble(x)/dble(gridx)*2*pi)*cos(dble(y)/&
            dble(gridy)*2*pi)*cos(dble(z)/dble(gridz)*2*pi))
        density(x,y,z,2) = block_lengthB*(1.0d0 - 0.5d0*cos(dble(x)/dble(gridx)*2*pi)*cos(dble(y)/&
            dble(gridy)*2*pi)*cos(dble(z)/dble(gridz)*2*pi))
        end do
     enddo
  end do
...
!
      open(unit=70, file=trim(output_path)//'energy.dat', status='unknown', &
          action='write', iostat=io_error)
      open(unit=71, file=trim(output_path)//'density.dat', status='unknown', &
          action='write', iostat=io_error)
!
...
!  cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
     do z = 1,gridz
        do y=1,gridy
           do x=1,gridx
                  write(71,*) zr(z), '  ' ,(saddledensity(x,y,z,i),i=1,monomer_types),'  ', &
                      (saddlefield(x,y,z,i),i=1,monomer_types)
              end do
           end do
        end do
...
!
    ...
    ! do i =0, gridz
    !     temp_saddle_energy(i)=dvol(i)*sum(saddledensity(:,:,i,:)*saddlefield(:,:,i,:))
    ! end do
    ! chain_energy = diblock%energy - sum(temp_saddle_energy)
    chain_energy = diblock%energy - saddle_energy 
    energy = chain_energy + interaction_energy(density) + external_energy(density)
    ! print*,'chain E',chain_energy, 'saddleE', sum(temp_saddle_energy)
    return
